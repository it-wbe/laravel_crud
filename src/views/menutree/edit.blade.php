...
@section('content')
    <link rel="stylesheet" type="text/css"
          href="/packages/wbe/crud/assets/admin_lte/libs/AdminLTE/plugins/select2/select2.css">
    <style>
        .controll {
            margin: 5px;
        }
    ...
    @if(isset($edit))
        <form method="post" class="form" id="new_node">
                ...
                @if(isset($langs[$edit_val['lang_id']]))
                    <div class="col-md-3">
                        <div class="form-group">
                            ...
                            <input type="text" class="form-control" name="title[{{$edit_val['lang_id']}}][title]"
                                   value="{{$edit_val['title']}}" required id="title">
                            <input type="hidden" name="title[{{$edit_val['lang_id']}}][lang_id]"
                                   value="{{$edit_val['lang_id']}}" required>
                        </div>
                    </div>
                @else
                    @continue
            ...
            <div class="col-md-12 controll">
                <button type="submit"
                        class="btn btn-primary col-md-4 col-xs-4">{!! __("crud::common.menu_edit_node") !!}</button>
                <a href="{{route('Menu Edit')}}"
                   class="btn btn-default col-md-4 col-xs-4 pull-right">{!! __("crud::common.back") !!}</a>
            </div>
        </form>
    @else
        ...
        <button onclick="location.href='{{route('Menu Edit')}}'"
                class="btn btn-default col-md-4 col-xs-4">{!! __("crud::common.cancel") !!}</button>
        <form method="post" class="form">
            {{ csrf_field() }}
            <button type="submit"
                    class="btn btn-primary col-md-4 col-xs-4 pull-right">{!! __("crud::common.delete") !!}</button>
        </form>
    @endif
@endsection
